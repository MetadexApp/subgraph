...
function getMV_RFV(transaction: Transaction): BigDecimal[] {
  ...
  let gyroUsdtPair = UniswapV2Pair.bind(Address.fromString(PCS_GYROBUSD_PAIR));
  let gyroBusdPair = UniswapV2Pair.bind(Address.fromString(PCS_GYROBUSD_PAIR));

  let treasury_address = TREASURY_ADDRESS;

  let usdtBalance = busdERC20.balanceOf(Address.fromString(treasury_address));//usdtERC20.balanceOf(Address.fromString(treasury_address));
  ...
  let gyroUsdt_value = getPairUSD(gyroUsdtBalance, PCS_GYROBUSD_PAIR);
  let gyroUsdt_rfv = getDiscountedPairUSD(gyroUsdtBalance, PCS_GYROBUSD_PAIR);
  */
  ...
  let stableValue = busdBalance.plus(busdBalance);
  let stableValueDecimal = toDecimal(stableValue, 18);

  let lpValue = gyroBusd_value.plus(gyroBusd_value);
  let rfvLpValue = gyroBusd_rfv.plus(gyroBusd_rfv);
  ...
  log.debug("Treasury GYRO-USDT RFV {}", [gyroBusd_rfv.toString()]);
  log.debug("Treasury BUSDT value {}", [toDecimal(busdBalance, 18).toString()]);
  log.debug("Treasury MDEX-BUSD RFV {}", [gyroBusd_rfv.toString()]);
  ...
  return [
    ...
    gyroBusd_rfv.plus(toDecimal(busdBalance, 18)),//gyroUsdt_value.plus(toDecimal(usdtBalance, 18)),
...
function getCriculatingSupply(): BigDecimal {
  let gyro_contract = GyroERC20.bind(Address.fromString(GYRO_ERC20_CONTRACT));
  let circ_supply = toDecimal(gyro_contract.totalSupply(), 9);
...
export function updateProtocolMetrics(transaction: Transaction): void {
  ...
  //pm.treasuryUsdtMarketValue = mv_rfv[3];//mv_rfv[4];
